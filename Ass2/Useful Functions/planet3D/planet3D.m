...
%
%==========================================================================
function planet_surface = planet3D(planet,opts, pos)
    ...
    factors = {'AU'   1/149597870000;
               ...
               'p'    2;
               'j'    1/4;
               }; %l'ultima non ha senso, l'ho aggiunta per vedere i pianeti grandi
    ...
    if ~strcmpi(planet,'Earth Coastlines')
        ...
        elseif strcmpi(planet,'Earth Night Cloudy')
            cdata = imread('earthnight.png')+...
                0.1*imread('clouds.png');
        else
            cdata = imread(strcat(lower(planet),'.png'));
        end

        % draws planet
        planet_surface = surface(x+pos(1),y+pos(2),z+pos(3),'facecolor','texture',...
    ...
    if strcmpi(planet,'Saturn')
        ...
        for i = 1:n_new
            ...
            x_ring = position(1)+pos(1)+r*cos(theta);
            y_ring = position(2)+pos(2)+r*sin(theta);
            z_ring = (position(3)+pos(3))*ones(size(theta));
