...
T3tspan = linspace(real.mjd2000_start*24*3600, real.mjd2000_start*24*3600 + 3*365*3600*24, 40000);

% propagation of Cartesian elements
[per.time_car, per.car] = ode113( @(t,s) ode_perturbed ...
    (t, s, astro.muE,  astro.RE, astro.J2, astro.cr, astro.A2m), T3tspan, [nominal.r0; nominal.v0], options.ode ); 

% propagation of Keplerian elements
[per.time_kep, per.kep_gauss] = ode113( @(t,s) gauss_propagation...
    (t, s, astro.muE, astro.RE, astro.J2, astro.cr, astro.A2m), T3tspan, nominal.kep', options.ode); 

% Keplerian elements to cartesian elements
for j = 1:length(T3tspan)
...
switch choice.filtering1
    case 1
        par = 1;
        % number of elements in the movemean
        k_par = nominal.T/3600/nominal.T*length(T3tspan);
...
filt.kep_gauss1 = movmean(per.kep_gauss(:,par), k_par);


switch choice.filtering2
    case 1
        name = 'i';
        par = 3;
        % number of elements in the movemean
...
filt.kep_gauss2 = movmean(per.kep_gauss(:,par), k_par);
