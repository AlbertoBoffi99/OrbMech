...
legend('Unperturbed', 'start unperturbed', 'end unperturbed', ...
    'Repeating unperturbed', 'start repeating unperturbed', 'end repeating unperturbed',...
    'Perturbed', 'start perturbed', 'end perturbed');
...
legend('Repeating unperturbed', 'start repeating unperturbed', 'end repeating unperturbed', ...
    'Repeating perturbed', 'start repeating perturbed', 'end repeating perturbed');
...
for i = [1 2]
    ...
    figure();
    plot((T3tspan-real.mjd2000_start*24*3600)/nominal.T, abs(per.kep_unwr(:,par) - per.kep_gauss(:,par))/norm);
    grid on
    % title ("relative error of 'name' between gauss and cartesian propagation")
    
    figure();
    plot((T3tspan-real.mjd2000_start*24*3600)/nominal.T, per.kep_gauss(:,par));
    hold on
    plot((T3tspan-real.mjd2000_start*24*3600)/nominal.T, per.kep_unwr(:,par));
    grid on
...
for i = 1 : 4
    ...
    figure();
    plot ((T3tspan-real.mjd2000_start*24*3600)/nominal.T, rad2deg(abs(per.kep_unwr(:,par) - per.kep_gauss(:,par)))./abs(norm));
    grid on
    titlestr = strcat("Relative error of ", name, " between Gauss and cartesian propagation");
    title (titlestr)
    
    figure();
    plot ((T3tspan-real.mjd2000_start*24*3600)/nominal.T, rad2deg(per.kep_gauss(:,par)));
    hold on
    plot ((T3tspan-real.mjd2000_start*24*3600)/nominal.T, rad2deg(per.kep_unwr(:,par)));
    grid on
    legend ('Gauss', 'Cartesian')
...
gaussvsreal = figure();
plot ((temp.tspan-real.mjd2000_start*24*3600)/nominal.T, per.kep_gauss(:,1), ...
    (temp.tspan_real-real.mjd2000_start*24*3600)/nominal.T, real.kep(:,1));
grid on
legend ('Gauss', 'Real')
...
plot ((T3tspan-real.mjd2000_start*24*3600)/nominal.T, per.kep_gauss(:,par));
hold on
plot ((T3tspan-real.mjd2000_start*24*3600)/nominal.T, filt.kep_gauss1);
grid on
...
plot ((T3tspan-real.mjd2000_start*24*3600)/nominal.T, rad2deg(per.kep_gauss(:,par)));
hold on
plot ((T3tspan-real.mjd2000_start*24*3600)/nominal.T, rad2deg(filt.kep_gauss2));
grid on
legend ('Original', 'Filtered')