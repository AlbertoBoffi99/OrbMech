...
clabel(C1,h1);                                              
% 
% non ci interessa l'orbita del singolo arco, secondo me si puo togliere
%
% TJ = 2*pi*sqrt( (kep1(1))^3/astro.muS );              
% tspanJ = linspace( 0, TJ, 1000 );
% [timeJ, stateJ] = ode113( @(t,s) tbp_ode(t, s, astro.muS), tspanJ, [r1(:,r), v1(:,r)], options.ode_options);
% 
% TE = 2*pi*sqrt( (kep2(1))^3/astro.muS );            
% tspanE = linspace( 0, TE, 1000 );
% [timeE, stateE] = ode113( @(t,s) tbp_ode(t, s, astro.muS), tspanE, [r2(:,c), v2(:,c)], options.ode_options);
% 
% tspan_t1 = linspace( 0, T11(r,c), 1000 );
% [time_t1, state_t1] = ode113( @(t,s) tbp_ode(t, s, astro.muS), tspan_t1, [r1(:,r), [Vi1(r,c,1) Vi1(r,c,2) Vi1(r,c,3)]'], options.ode_options);
% 
% figure()          
% plot3( stateJ(:,1), stateJ(:,2), stateJ(:,3), 'blue', 'linewidth', 2)     
% hold on
% plot3( state_t1(:,1), state_t1(:,2), state_t1(:,3), 'green--', 'linewidth', 2)
% plot3( stateE(:,1), stateE(:,2), stateE(:,3), 'red', 'linewidth', 2)    
% 
% opts.units= 's';
% planet3D('Sun',opts, [0 0 0]);  
% 
...
a_day= mj20002date(results.fmincon_dates(3));
...
clabel(C1,h1);                                              
% 
% di nuovo, non ci interessa questo
%
% TE = 2*pi*sqrt( (kep2(1))^3/astro.muS );            
% tspanE = linspace( 0, TE, 1000 );
% [timeE, stateE] = ode113( @(t,s) tbp_ode(t, s, astro.muS), tspanE, [r2(:,r), v2(:,r)], options.ode_options);
% 
% TV = 2*pi*sqrt( (kep3(1))^3/astro.muS );              
% tspanV = linspace( 0, TV, 1000 );
% [timeV, stateV] = ode113( @(t,s) tbp_ode(t, s, astro.muS), tspanV, [r3(:,c), v3(:,c)], options.ode_options);
% 
% tspan_t2 = linspace( 0, T22(r,c), 1000 );
% [time_t2, state_t2] = ode113( @(t,s) tbp_ode(t, s, astro.muS), tspan_t2, [r2(:,r), [Vi2(r,c,1) Vi2(r,c,2) Vi2(r,c,3)]'], options.ode_options);
% 
% figure()          
% plot3( stateE(:,1), stateE(:,2), stateE(:,3), 'blue', 'linewidth', 2)      
% hold on
% plot3( state_t2(:,1), state_t2(:,2), state_t2(:,3), 'green--', 'linewidth', 2) 
% plot3( stateV(:,1), stateV(:,2), stateV(:,3), 'red', 'linewidth', 2)      
% 
% opts.units= 's';
% planet3D('Sun',opts, [0 0 0]);  
% 
...
fb_time = time1 + time2;
%non abbiamo questi risultati dal problema generale
...
% and showing the perigee passage
